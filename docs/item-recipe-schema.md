...
- **Validación profunda de recetas** (¿existen realmente los ids de ingredientes referenciados en el catálogo? ¿hay dependencias circulares?) — esta entrega solo valida la *forma* de `recetaMatriz` (tamaño, compuerta de materia prima). La propuesta original nombraba esto como su propia característica futura, "Validación y almacenamiento de recetas"; en el roadmap actualizado (ver "Qué Sigue" más abajo) esa validación referencial ya no es un ítem separado — queda absorbida dentro del alcance de la futura **Calculadora de crafteo** (ítem 3 del roadmap), que de todas formas necesitará resolver cada id de `ingredientesParaCalculo` contra el catálogo real para poder agregar cantidades.
...
- **Una capacidad de traducción español↔inglés de propósito general** — el diccionario solo cubre el vocabulario de items conocido de este proyecto (48 entradas), no texto arbitrario.
...
3. **Calculadora de crafteo** — UI interactiva para planificar cadenas de crafteo; mostrar cantidades de salida. Consume directamente `recetaMatriz`/`ingredientesParaCalculo` de esta entrega, y absorbe el alcance de validación referencial que la propuesta original de esta entrega había nombrado por separado como "Validación y almacenamiento de recetas" (ver "Explícitamente Fuera de Alcance" más arriba) — resolver cada id de ingrediente contra el catálogo real es un prerrequisito natural de la agregación de cantidades que hace esta calculadora.
